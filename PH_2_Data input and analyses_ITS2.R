...
otu.PH.tmp <- read.table("../../Data/ITS2/otu.ITS.1200.PH.csv", header=TRUE, sep=',', row.names = 1)
otu.PH <- otu.PH.tmp[, apply(otu.PH.tmp, 2, sum) > 0]/1200 # remove OTUs with no seqs and divide by 1200 to get relative abundance (0-1)

### Load the taxonomy information into the environment
tax.PH <- read.table("../../Data/ITS2/taxonomy.ITS.PH.csv", header=TRUE, sep=',', row.names = 1)

### Load the environmental metadata tables with alpha diversity into the environment and set factors
env.PH <- read.table("../../Data/ITS2/env.ITS.1200.PH.csv", header=TRUE, sep=',', row.names = 1)
...
## Outliers
outliers <- c("PH.S1.GN.BS.2","PH.S1.L.BS.2","PH.S1.PA.BS.7","PH.S1.L.ER.8")
env.PH <- env.PH[!row.names(env.PH) %in% outliers,]
otu.PH <- otu.PH[!row.names(otu.PH) %in% outliers,]
